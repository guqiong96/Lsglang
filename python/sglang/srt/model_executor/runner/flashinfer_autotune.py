...
def _tp_group_min_sm_major(model_runner: ModelRunner) -> int:
    ...
    heterogeneous groups (e.g. mixed SM86 + SM120): the SM<9 ranks would skip
    autotune and proceed to cuda-graph capture (blocking in a capture barrier)
    while the SM>=9 ranks enter autotune and block in its all_gather. Resolve
    the gate over the whole TP group so every rank agrees.
    """
