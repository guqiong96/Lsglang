...
still fit. The storage backend does not change NUMA allocation policy.
An independent, default-off [SM120 MXFP8 option](README.sm120_mxfp8.md) can
accelerate decode and fixed-width DSpark verification linears.
...
- One worker (`TP=PP=1`, world size 1), one running request, overlap scheduling
  disabled and no `torch.compile`. Ordinary decode and fixed-width DSpark with
  gamma 3 or 5 (verification width 4 or 6) are enabled. Other speculative
  algorithms, compact/ragged mode and inconsistent DSpark widths are rejected.
  Unsupported
...
- CUDA graph staging must be warmed on the capture stream. Decode graph BS=1
  plus fixed-width verification graphs and eager Prefill are component-tested.
  Concurrent graph replay on
...
Keep the constraints above and set `SGLANG_RAGGED_VERIFY_MODE=static`.
For gamma 5 use `--speculative-algorithm DSPARK
--speculative-dspark-block-size 5 --speculative-num-draft-tokens 6`; for gamma 3
use block size 3 and draft tokens 4. The checkpoint draft configuration must
also match the chosen gamma. Draft weight residency remains an existing
Lsglang configuration choice, not a change to the storage or acceptance algorithm.

The CUDA test exercises the actual `EngramHasher` and `EngramEmbedding`:
CPU-reference hash IDs, exact row dequantization, both graph widths, alternating
24 graph replays with changing IDs/history, zero through full acceptance commits,
and isolation of untouched request histories. Verify must not commit history;
only the existing acceptance commit method updates it. This is a synthetic
component integration test, **not a full-model DSpark quality/throughput test**.
...
The DSpark integration adds another 24 graph replays across widths 4 and 6.
