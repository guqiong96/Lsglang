...
class TestMetadata(unittest.TestCase):
    ...
    def test_dspark_configuration_guards(self):
        ...
        for gamma, width in [(3, 4), (5, 6)]:
            ...
            with (
                ...
                validate_configuration(parallel, schedule, spec, graph, False)
                with (
                    patch.object(spec, "speculative_num_draft_tokens", width - 1),
                    self.assertRaises(ValueError),
                ):
                    validate_configuration(parallel, schedule, spec, graph, False)
                for mode in ["compact", "auto", "invalid"]:
                    with (
